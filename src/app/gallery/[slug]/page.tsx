  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        {project.images.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
            {project.images.map((image, index) => (
              <div
                key={image}
                className="aspect-square bg-white rounded-xl overflow-hidden cursor-pointer hover:ring-2 hover:ring-primary transition-all duration-300"
        ...
        {project.images.length === 0 && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div
                key={i}
                className="aspect-square bg-white rounded-xl flex items-center justify-center border-2 border-dashed border-gray-300"
              >
      ...
      {selectedImage !== null && project.images.length > 0 && (
        <div className="fixed inset-0 z-50 bg-gray-100/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="relative max-w-6xl w-full h-full flex items-center justify-center">
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 z-10 w-12 h-12 bg-gray-100/70 hover:bg-gray-100/90 text-gray-900 rounded-full flex items-center justify-center transition-colors duration-300"
            >
              <X size={24} />
            </button>
            ...
            <button
              onClick={prevImage}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 z-10 w-12 h-12 bg-gray-100/70 hover:bg-gray-100/90 text-gray-900 rounded-full flex items-center justify-center transition-colors duration-300"
            >
              <ChevronLeft size={24} />
            </button>
            ...
            <button
              onClick={nextImage}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 z-10 w-12 h-12 bg-gray-100/70 hover:bg-gray-100/90 text-gray-900 rounded-full flex items-center justify-center transition-colors duration-300"
            >
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-gray-100/70 text-gray-900 px-4 py-2 rounded-full">
