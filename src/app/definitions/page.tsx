...
export const metadata: Metadata = {
  title: 'Acoustic & Soundproofing Glossary | Supersonic Customs South Africa',
  description: 'Plain-language definitions of acoustic and soundproofing terms by South Africa\'s specialists - STC, NRC, decoupling, floating floors, RT60, mineral wool, mass loaded vinyl and more. Understand the science before you spend.',
  alternates: {
    canonical: "https://supersoniccustoms.co.za/definitions",
  },
  openGraph: {
    title: 'Acoustic & Soundproofing Glossary | Supersonic Customs South Africa',
    description: 'Plain-language acoustic definitions from South Africa\'s soundproofing specialists. Understand STC, NRC, decoupling and more before you invest.',
        ...
        <div className="max-w-5xl mx-auto flex flex-wrap gap-2">
          {categories.map(cat => (
            <a key={cat} href={`#${cat.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}
              className="px-3 py-1 bg-gray-200 hover:bg-blue-600 text-gray-600 hover:text-gray-900 rounded-full text-xs font-medium transition-colours">
              {cat}
            </a>
          ))}
        </div>
          ...
          <div key={cat} id={cat.toLowerCase().replace(/[^a-z0-9]+/g, '-')} className="mb-14">
            ...
            <div className="space-y-6">
              {definitions.filter(d => d.category === cat).map(def => (
                <div key={def.term} id={def.term.toLowerCase().replace(/[^a-z0-9]+/g, '-')} className="bg-white/50 rounded-2xl p-6 border border-gray-200 hover:border-gray-300 transition-colours">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      {def.related.map(rel => (
                        <a key={rel} href={`#${rel.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}
                          className="px-2 py-1 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded text-xs transition-colours">
                          {rel}
                        </a>
                      ))}
                    </div>
